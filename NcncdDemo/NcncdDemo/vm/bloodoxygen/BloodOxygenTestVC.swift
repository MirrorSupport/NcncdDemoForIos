...
class BloodOxygenTestVC: UIViewController, NcncdApiDelegate {
    ...
    @IBAction func actionAdd(_ sender: Any) {
        ...
        if !checkTfText(tfView: tfAverage, notice: "平均浓度格式错误") {
            return
        }
        
        if !checkTfText(tfView: tfMax, notice: "最大浓度格式错误") {
            return
        }
        
        if !checkTfText(tfView: tfMin, notice: "最低浓度格式错误") {
            return
        }
    ...
    func addData(average: Float, max: Float, min: Float, time: Int) {
        ...
        bloodOxygen.average = average
        bloodOxygen.max = max
        bloodOxygen.min = min
        bloodOxygen.beginTime = Int(Date().timeIntervalSince1970) - time
        bloodOxygen.endTime = Int(Date().timeIntervalSince1970)
        
        datas.append(bloodOxygen)
        
        var text = "平均浓度：\(average) , 最大浓度：\(max) , 最低浓度：\(min)"
    ...
    @IBAction func actionUpload(_ sender: Any) {
        ...
        BloodOxygenApi.delegate = self
        BloodOxygenApi.uploadBloodOxygen(data: datas, telephone: "13645787776")
